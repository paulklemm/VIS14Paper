...
Generalized Pairs Plot (\emph{GPLOMS}) is an information visualization technique that allows for heterogenous variables to be pairwise visualized using appropriate plots in a plot matrix grouped by type \cite{Francois2013}.
%
...
\end{figure}
IVA levels defines different interaction levels. 
%
We employ the fourth and highest levels which includes next to brushing and linking, advanced brushing, extraction of attributes the development of visualizations custom tailored to the data sets.
%
...
\subsection{Adaptive Feature Visualization}
...
\end{figure}
%
In order to provide a fast communication loop between method development and expert input we decided to base all implementations on modern web technologies which employs various advantages.
\begin{itemize}
...
%
Image data is available as raw MRI DICOM files as well as segmented Meshes which can be used to compare subjects.
%
