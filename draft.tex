...
\author{P. Klemm$^{1}$, K. Lawonn$^{1}$, M. Rak$^{1}$, B. Preim$^{1}$, K. Toennies$^{1}$, K. Hegenscheid$^{2}$, H. Völzke$^{2}$, S. Oeltze$^{1}$}
\authorfooter{
...
\\\\
\section{Application}
\add{This Chapter needs to be reworked completely, since it was subject of the reviewer criticism.}
...
We conducted a case study with two epidemiological domain experts.
%
...
\subsection{Hypothesis-free analysis}
...
%
The latter was chosen by both domain experts.
%
They were excited by the possibility to look at all variables in the context of each other.
%
They stated that it "illumunates the black box of the data", making it possible to look at the data in an unbiased way.
%
...
After this initial overview, both experts performed a \emph{multivariate analysis} by introducing features, such as age, waist circumference, weight or lumbar spine centerline curvature as bar charts views into the canvas area and selected subgroups to see how they are distributed and if they could observe unusual behavior in the mean-shapes.
%

% TODO: Contingency Matrix or Adjecency Matrix
\com{00.44 Clustering-Ergebnis Weiblich und Arbeitspensum Alter}

\subsection{Hypothesis-based analysis}
The radiologist performed a \emph{local investigation} by dragging the age variable into the canvas area, observing that age-bins 36-52 years and 52-68 years are nearly identical to the global mean, while 20-36 years and 68-84 years differ in size.
%
...
A second analysis was carried out on body weight.
...
%
The radiologist pointed out that the image data representations would be better comparable if they would be abstracted into representative land marks, such as centers of the vertebrae and cardinal points of the lumbar spine canal.
%
